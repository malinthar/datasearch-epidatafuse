...
public class DengueData {
    ...
    public List<SimpleFeature> getTestData() {
        if (features == null) {
            ...
            URL input = getClass().getClassLoader().getResource("model-data.CSV");
            if (input == null) {
                throw new RuntimeException("Couldn't load resource model-data.CSV");
            }
            ...
            try (CSVParser parser = CSVParser.parse(input, StandardCharsets.UTF_8, CSVFormat.TDF)) {
                for (CSVRecord record : parser) {
                    try {
                        ...
                        // however, the date format here isn't one that is converted,
                        // so we parse it into a java.util.Date
                        builder.set("dtg",
                                Date.from(LocalDate.parse(record.get(1),
